...
- String(문자열)
    ...
    * 메서드 : 
        ...
        * str.repeat()
        ...
        'hello'.repeat(100)
        '0'.repeat(100)
        '5'.repeat(100).split('').map(Number) //여기서 Number는 콜백함수
        ```
...
- Array(배열) : object
    ...
    * 메서드 : 
        ...
        * sort — mutable
        ...
        mutable 원본이 수정가능한 메서드, immutable은 원본이 수정이 불가능한 메서드
    ```js
    Array(100).fill(0)
...
//예제
let a = [10, 20, 30, 40];
for (let i of a) {  // for of는 배열의 요소를 가져온다
    console.log(i);
}
```
```js
//예제
let a = [10, 20, 30, 40];
for (let i in a) {  // for in는 배열의 키를 가져온다
    console.log(i);
}
let a = {'one':1, 'two':2};
for (let i in a) {
...
- 함수 표현식과 함수 선언식
    ```js
    let 함수표현식 = function(){} // 호이스팅 X, 실제로 호이스팅은 되지만 일시적 사각지대(TDZ)에 빠지기 때문에 안되는 것처럼 보인다.
